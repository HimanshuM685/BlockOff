...
// Default to a public RPC to avoid Infura; override via SEPOLIA_RPC_URL if needed.
const RPC_URL = process.env.SEPOLIA_RPC_URL ?? "https://sepolia.drpc.org";
const CONTRACT_ADDRESS = process.env.SEPOLIA_TOKEN_CONTRACT_ADDRESS ?? ""; // Set your Sepolia contract address
...
(async () => {
  try {
    console.log("🚀 Starting Sepolia transaction relayer script...");
    ...
    console.log(`Relayer Balance: ${ethers.formatEther(balance)} native tokens`);
