...
(async () => {
  ...
  // Use env-provided key and addresses for Sepolia
  ...
  const TO_ADDRESS = process.env.SEPOLIA_TO_ADDRESS ?? "";
  if (!OWNER_PRIVATE_KEY) throw new Error("Set SEPOLIA_OWNER_PRIVATE_KEY");
  if (!TOKEN_CONTRACT_ADDRESS) throw new Error("Set SEPOLIA_TOKEN_CONTRACT_ADDRESS");
  if (!TO_ADDRESS) throw new Error("Set SEPOLIA_TO_ADDRESS");
  ...
  const domain = {
    name: "somehow", // Set to your token's name on Sepolia
