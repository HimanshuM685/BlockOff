...
export const TransactionLoader: React.FC<TransactionLoaderProps> = ({
  ...
  const { broadcastMessage, masterState, getProgressFor } = useBle();
  ...
  // Get chunk progress for display - use BLE context's getProgressFor for consistency
  const getChunkProgress = useCallback((): {
    ...
    if (broadcastId && masterState.has(broadcastId)) {
      const state = masterState.get(broadcastId);
      if (state) {
        // Use the BLE context's getProgressFor which handles both outgoing and incoming messages
        return getProgressFor(state);
      }
    }
    return { received: 0, total: 0 };
  }, [broadcastId, masterState, getProgressFor]);
  ...
  const handleTransactionSigning = async () => {
    try {
      ...
      try {
        ...
        const states = Array.from(masterState.entries()) as [number, MessageState][];
