 ...
 */
export async function getBalance(address: string, chainId: string = 'sepolia'): Promise<string> {
  try {
    const chain = getChainById(chainId);
    if (!chain) {
 ...
 */
export async function getCurrentGasPrice(chainId: string = 'sepolia'): Promise<string> {
  try {
    const chain = getChainById(chainId);
    if (!chain) {
      throw new Error(`Unsupported chain: ${chainId}`);
 ...
 */
export function getChainInfo(chainId: string = 'sepolia') {
  const chain = getChainById(chainId);
  if (!chain) {
    throw new Error(`Unsupported chain: ${chainId}`);
  }
...
// Export Sepolia testnet info for backward compatibility
export const TESTNET_INFO = getChainInfo('sepolia');
